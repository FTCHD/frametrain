                ...
                ...storage.bids.map((bid, index) => ({
                    ...
                    props: {
                        ...
                        children: [
                            ...
                            {
                                type: 'p',
                                props: { children: `Amount: ${bid.amount} ${config.currency}` },
                            },
                            {
                                type: 'p',
                                props: {
                                    children: `Time: ${new Date(bid.timestamp).toLocaleString()}`,
                                },
                            },
                            { type: 'hr' },
                        ],
...
export function handleAuctionEnd(storage: Storage, config: Config): BuildFrameData {
    if (storage.bids.length === 0) {
        throw new Error('No bids available to end the auction.')
    }
...
export function handleBidAcceptance(
    storage: Storage,
    config: Config,
    bidderToAccept: string
): BuildFrameData {
...
const commonStyle = { fontFamily: 'Roboto', fontSize: '24px', textAlign: 'center', padding: '20px' };
    ...
    const adUpdatedComponent = {
        ...
        props: {
            ...
            children: [
                ...
                {
                    type: 'p',
                    props: { children: `Your new visit link: ${storage.currentAd.visitLink}` },
                },
            ],
        },
    }
