...
import getSubstackArticle from './utils'
...
export default function Inspector() {
    ...
    const urlInputHandler = async (url: string) => {
        ...
        if (!/^(https?:\/\/[^\s]+)/.test(url)) {
            toast.error('Please enter a valid substack article URL')
            return
        }

        try {
            setLoading(true)
            const newArticle = await getSubstackArticle(url)
            updateConfig({ article: newArticle })
            console.log(newArticle)
            toast.success('Successfully fetched the substack article')
        } catch (e) {
    ...
    return (
        <Configuration.Root>
            <Configuration.Section title="Substack Article URL">
                <div className="flex flex-row w-full gap-2">
                    <Input
                        className="py-2 text-lg"
                        placeholder="https://on.substack.com/..."
                        ref={urlInputRef}
