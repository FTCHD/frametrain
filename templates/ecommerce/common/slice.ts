...
type ProductCart = any

const getProduct = async (slicerId: number, productId: number) => {
    const response = await corsFetch(`https://slice.so/api/slicer/${slicerId}/product/${productId}`)
    return response
}

const getStoreProducts = async (slicerId: number) => {
    const response = await corsFetch(`https://slice.so/api/slicer/${slicerId}/products`)
    return response
}

const payProductsConfig = async (slicerId: number, cart: ProductCart[], account: `0x${string}`) => {
    const response = await corsFetch(`https://slice.so/api/slicer/${slicerId}/pay`, {
        method: 'POST',
        body: JSON.stringify({ cart, account }),
    })
    return response
}
