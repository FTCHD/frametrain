...
export function parseFrameHtml(html: string) {
  ...
  const frameMetaTagsProperty = document.querySelectorAll(
    `[property^='fc:frame']`
  );
...
export function frameResultToFrameMetadata(
  ...
  const refreshPeriod = rawRefreshPeriod
    ? Number.parseInt(rawRefreshPeriod, 10)
    : undefined;
  const state = rawState
    ? JSON.parse(decodeURIComponent(result["fc:frame:state"]))
    : undefined;
...
export const vNextSchema = z
  .object({
    "fc:frame": z
      .string()
      .regex(/vNext/, { message: '"fc:frame" must be "vNext"' }),
